...
const buildDetailedScheduleLabel = (rawLabel = '', rawType = '', transmission = '') => {
  ...
  const isExplicitMT = txCodeRaw === 'MT' || txCodeRaw === 'MANUAL';
  const isExplicitAT = txCodeRaw === 'AT' || txCodeRaw === 'AUTOMATIC';
  const hasManual = isExplicitMT || (!isExplicitAT && (source.includes(' MANUAL ') || source.endsWith(' MANUAL') || source.includes('(MANUAL)') || /(^|\W)MT($|\W)/.test(source)));
  const hasAutomatic = isExplicitAT || (!isExplicitMT && (source.includes(' AUTOMATIC ') || source.includes(' AUTO ') || source.endsWith(' AUTOMATIC') || source.includes('(AUTOMATIC)') || /(^|\W)AT($|\W)/.test(source)));
