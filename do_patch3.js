const fs = require('fs');
const fp = 'C:/Users/gabas/OneDrive/Desktop/Booking System/booking-system-frontend/src/admin/config/EmailContentSection.jsx';
let content = fs.readFileSync(fp, 'utf8');

const typesStr = `    const emailTypes = [
...
const gridStr = `        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {emailTypes.map(type => {
...
if (!content.includes('const Icons = {')) {
    content = content.replace(/(export default function EmailContentSection\(\) \{)/, iconStr + '\n\n$1');
}

// target regular expression
const regexTypes = /    const emailTypes = \[[\s\S]*?\];/;
content = content.replace(regexTypes, typesStr);

const regexGrid = /<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">[\s\S]*?<\/div>(\s*)\{\/\* Email Edit Modal \*\/\}/;
content = content.replace(regexGrid, gridStr + '$1{/* Email Edit Modal */}');
