...
const path = 'C:/Users/gabas/OneDrive/Desktop/Booking System/booking-system-frontend/src/admin/config/EmailContentSection.jsx';
...
content = content.replace(/const IconLink/g, iconsStr + "\nconst IconLink");

const oldTypesStr = \    const emailTypes = [
        { id: 'walkIn', title: 'Walk-In Enrollment', desc: 'Sent to walk-in students with credentials.' },
        { id: 'guest', title: 'Guest Enrollment', desc: 'Sent to guests enrolling themselves.' },
        { id: 'noShow', title: 'No-Show / Missed', desc: 'Sent when a student misses a session.' },
        { id: 'news', title: 'News & Promos', desc: 'Promotional broadcast emails.' },
        { id: 'receipt', title: 'Payment Receipt', desc: 'Sent after successful transactions.' },
        { id: 'newAccount', title: 'New Admin Account', desc: 'Sent to new staff members.' },
        { id: 'verification', title: 'Verification & OTP', desc: 'Sent when users register or reset passwords.' }
    ];\;
...
content = content.replace(oldTypesStr, newTypesStr);

const oldGridStr = \            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {emailTypes.map(type => (
                    <div key={type.id} onClick={() => setSelectedEmail(type.id)} className="bg-white border border-gray-200 rounded-2xl p-6 flex flex-col items-center text-center cursor-pointer hover:shadow-xl hover:border-blue-400 hover:-translate-y-1 transition-all duration-200 group">
                        <div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mb-5 group-hover:scale-110 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300 shrink-0 shadow-sm">
                            <IconMail />
                        </div>
                        <h3 className="font-bold text-gray-800 text-lg mb-2 group-hover:text-blue-700 transition-colors">{type.title}</h3>
                        <p className="text-sm text-gray-500 leading-relaxed px-2">{type.desc}</p>
                        
                        <div className="mt-5 w-full pt-4 border-t border-gray-100 flex items-center justify-center text-sm font-semibold text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity">
                            Edit Template &rarr;
                        </div>
                    </div>
                ))}
