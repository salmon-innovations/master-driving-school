...
const getZipForBranch = (name) => {
    if (!name) return '';
    const norm = name.toLowerCase();
    for (const [branch, zip] of Object.entries(BRANCH_ZIP_CODES)) {
        if (norm.includes(branch.toLowerCase())) return zip;
    }
    return '';
};
        ...
        <div className="step-content animate-fadeIn space-y-8">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border-2 border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                    ...
                    {/* Middle Initial with Segmented Control */}
                    <div className="md:col-span-2 flex flex-col gap-2">
                        <label className="text-sm font-bold text-gray-700">Middle Initial</label>
                        <div className="flex flex-col gap-2">
                            <div className="flex bg-gray-100 p-1.5 rounded-xl border border-gray-200">
                                <button 
                                    type="button"
                                    onClick={() => setFormData(prev => ({ ...prev, middleName: '' }))}
                                    className={`flex-1 py-1.5 rounded-lg text-xs font-black transition-all ${formData.middleName !== 'N/A' ? 'bg-white text-[#2157da] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    Has
                                </button>
                                <button 
                            ...
                            <input 
                                ...
                                className={`w-full px-3 py-3.5 bg-gray-50 border-2 rounded-2xl text-center outline-none transition-all ${formData.middleName === 'N/A' ? 'bg-gray-100 border-gray-200 opacity-50' : 'border-gray-100 focus:border-[#2157da] focus:ring-4 focus:ring-blue-50'}`} 
                            />
                        </div>
                    </div>
