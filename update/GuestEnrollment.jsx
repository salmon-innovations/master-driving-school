...
function GuestEnrollment({ onNavigate, setIsLoggedIn, preSelectedBranch }) {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Auto-fill zip code based on address keywords
      if (name === 'address') {
        const norm = (formattedValue || '').toLowerCase().trim();
        
        if (!norm) {
          // If address is cleared, reset dependent fields
          updated.zipCode = '';
        } else {
          let matchFound = false;
          for (const [branch, zip] of Object.entries(BRANCH_ZIP_CODES)) {
            if (norm.includes(branch.toLowerCase())) {
              updated.zipCode = zip;
              matchFound = true;
              break;
            }
          }
          // If no branch keywords match, reset zipCode (shows placeholder)
          if (!matchFound) updated.zipCode = '';
        }
      }
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[url('/images/cover.png')] bg-cover bg-center bg-no-repeat py-12 px-4 relative font-sans">
      ...
      <div className="w-full max-w-6xl bg-white/95 backdrop-blur-md rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row relative z-10 min-h-[750px] border border-white/20" data-aos="zoom-in">

        {/* ──────────────────────────────────────────────────────────────────
            LEFT SECTION: FORM CONTENT
            ────────────────────────────────────────────────────────────────── */}
        <div className="w-full lg:w-3/5 p-8 lg:p-14 relative flex flex-col">
          <button
          ...
          <div className="mt-14 w-full">
            ...
            <div className="form-sections-container max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                          <label className="text-[0.65rem] font-black text-gray-400 uppercase tracking-widest mb-1.5 block ml-1">Middle Initial</label>
                          <div className="flex flex-col gap-2">
                            <div className="flex bg-gray-200/50 p-1 rounded-xl h-12">
                              <button
                                type="button"
                                onClick={() => setFormData(prev => ({ ...prev, middleName: '' }))}
                                className={`flex-1 rounded-lg text-xs font-bold transition-all duration-300 ${formData.middleName !== 'N/A' ? 'bg-white shadow-sm text-[#2157da]' : 'text-gray-500 hover:text-gray-700'}`}
                              >
                                Has Initial
                              </button>
                              <button
                                type="button"
                                onClick={() => setFormData(prev => ({ ...prev, middleName: 'N/A' }))}
                                className={`flex-1 rounded-lg text-xs font-bold transition-all duration-300 ${formData.middleName === 'N/A' ? 'bg-white shadow-sm text-[#2157da]' : 'text-gray-500 hover:text-gray-700'}`}
                              >
                                None
                              </button>
                            ...
                            <input
                              ...
                              className={`w-full px-4 py-3 bg-white border-2 border-transparent shadow-sm rounded-xl focus:border-[#2157da] focus:ring-4 focus:ring-blue-50 outline-none transition-all font-semibold ${formData.middleName === 'N/A' ? 'opacity-40 grayscale cursor-not-allowed bg-gray-50' : 'hover:border-gray-200'}`}
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <select
                              ...
                              onChange={handleChange}
                              className={`w-full px-4 py-3.5 bg-white border-2 border-transparent shadow-sm rounded-xl focus:border-[#2157da] focus:ring-4 focus:ring-blue-50 outline-none transition-all font-semibold appearance-none cursor-pointer ${errors.nationality ? 'border-red-300 ring-4 ring-red-50' : 'hover:border-gray-200'}`}
                            >
        ...
        {/* ──────────────────────────────────────────────────────────────────
            ...
            ────────────────────────────────────────────────────────────────── */}
        <div className="w-full lg:w-2/5 bg-[#1a3a8a] p-12 flex flex-col justify-center items-center text-white relative overflow-hidden">
          {/* Decorative Elements */}
            ...
            <div className="hidden lg:block space-y-12 ml-6 border-l-2 border-blue-400/30 pl-10 relative">
              {steps.map((step, index) => (
                <div key={step.id} className="relative group">
                  {/* Dot on line */}
                  <div className={`absolute -left-[51px] top-1 w-10 h-10 rounded-full border-4 border-[#1a3a8a] flex items-center justify-center transition-all duration-500 shadow-xl ${
