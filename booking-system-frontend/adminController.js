...
const {
  ...
  sendOnlineTdcAccountReadyEmail,
  reloadEmailContent,
  sendTestEmail,
...
const walkInEnrollment = async (req, res) => {
  ...
  try {
    ...
    const isOnlineTdcNoSchedule =
      String(courseCategory || '').toUpperCase() === 'TDC' &&
    ...
    try {
      ...
      notesCourseList = idsToFetch
        .map((id) => {
          ...
          if (!meta) return null;
          const selectedTdcType = String(courseType || '');
...
// Send Online TDC provider account-ready email on demand (admin action)
const sendOnlineTdcAccountEmail = async (req, res) => {
  try {
    const { id } = req.params;
    const {
      providerName,
    ...
    const result = await pool.query(
      `SELECT b.id, b.notes, b.course_type, b.booking_date,
              u.first_name, u.last_name, u.email,
    ...
    const searchable = `${row.course_name || ''} ${row.course_category || ''}`.toLowerCase();
    const hasTdcSignal = searchable.includes('tdc') || searchable.includes('theoretical');
    const hasOnlineSignalFromCourse = searchable.includes('online') || String(row.course_type || '').toLowerCase().includes('online');
    const hasOnlineSignalFromNotes = Boolean(notesJson?.isOnlineTdcNoSchedule || notesJson?.noScheduleRequired) ||
      (Array.isArray(notesJson?.courseList) && notesJson.courseList.some((item) => {
        const src = `${item?.name || ''} ${item?.type || ''} ${item?.category || ''}`.toLowerCase();
        return src.includes('tdc') && src.includes('online');
      }));

    if (!(hasTdcSignal && (hasOnlineSignalFromCourse || hasOnlineSignalFromNotes))) {
      return res.status(400).json({ error: 'This booking is not an Online TDC enrollment.' });
    }

    await sendOnlineTdcAccountReadyEmail(row.email, row.first_name, row.last_name, {
      providerName,
      providerLoginUrl,
      providerEmail,
      providerUsername,
    ...
    res.json({
      success: true,
      message: `Online TDC account email sent to ${row.email}`,
    });
  } catch (error) {
    console.error('Send Online TDC account email error:', error);
    res.status(500).json({ error: 'Failed to send Online TDC account email' });
  }
};
