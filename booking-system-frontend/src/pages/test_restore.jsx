...
function Schedule({ onNavigate, selectedCourse, preSelectedBranch, setScheduleSelection, cart, setCart, isLoggedIn }) {
  ...
  const promoPdcFiltered = promoPdcRawSlots.filter(promoPdcSlotMatches).sort((a, b) => {
    const order = { 'morning class': 1, 'afternoon class': 2, 'whole day class': 3, 'morning': 1, 'afternoon': 2, 'whole day': 3 };
    return (order[(a.session || '').toLowerCase()] || 99) - (order[(b.session || '').toLowerCase()] || 99);
  });
  
  promoPdcFiltered.sort((a,b) => {
    const o = {
      'morning class': 1, 'afternoon class': 2, 'whole day class': 3,
      'morning': 1, 'afternoon': 2, 'whole day': 3
    };
    return (o[(a.session || '').toLowerCase()] || 99) - (o[(b.session || '').toLowerCase()] || 99);
  });
// Day 2 slots must match vehicle type AND must match the same session as Day 1
  ...
  // Promo PDC min selectable date: TDC end_date + 2 days (student must finish TDC first)
    ...
    afterTdc.setDate(afterTdc.getDate() + 3); // Advance 2 clear days (PDC starts on 3rd day)
                                  ...
                                  return (
                                    <div
                                      ...
                                      className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                                        isSlotSelected ? 'bg-[#2563eb]' :
                                        isFullyBooked ? 'bg-red-400' :
                                        (slot.session||'').toLowerCase().includes('morning') ? 'bg-orange-400' :
                                        (slot.session||'').toLowerCase().includes('afternoon') ? 'bg-yellow-400' :
                                        'bg-blue-400'
                                      }`}
                                    />
                                  );
...
const isFull = slot.available_slots === 0
                  ...
                  return (
                    <button key={slot.id} onClick={() => handleSlotClick(slot)} disabled={isFull || isDay1} style={{ background: cardBg, borderColor: cardBorder }} className={`group relative flex flex-col p-4 text-left rounded-2xl border-2 transition-all overflow-hidden ${isFull ? 'opacity-60 cursor-not-allowed' : isDay1 ? 'shadow-xl shadow-blue-500/30 scale-[1.02]' : 'hover:shadow-md hover:-translate-y-0.5'}`}>
                      <div
                        className="h-1.5 w-full flex-shrink-0"
                        style={{ background: isDay1 ? 'rgba(255,255,255,0.3)' : isDay2 ? '#22c55e' : sessionMeta.color }}
                      />
                    ...
                    <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-5 sm:p-7 mb-4">
                      ...
                      <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
                        ...
                        {[...Array(promoPdcDay2DaysInMonth)].map((_, i) => {
                          ...
                          const today = new Date(); today.setHours(0, 0, 0, 0)
                          const isToday = cellDate2.getTime() === today.getTime()
                          const currentDayStr2 = `${promoPdcDay2CalMonth.getFullYear()}-${String(promoPdcDay2CalMonth.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`
                          const daySlots2 = promoPdcAllFiltered
                            .filter(s => s.date === currentDayStr2 && s.session === promoPdcSlot?.session)
                            .sort((a, b) => ({ morning: 1, afternoon: 2, 'whole day': 3 }[(a.session || '').toLowerCase()] || 99) - ({ morning: 1, afternoon: 2, 'whole day': 3 }[(b.session || '').toLowerCase()] || 99))
                          const hasRealSlots2 = daySlots2.length > 0
                          const hasAvailability2 = daySlots2.some(s => s.available_slots > 0)
                          const cellBorder2 = isDay1
                            ...
                            : isSel2
                              ...
                              : isToday
                                ...
                                : !avail
                                  ? 'border-transparent bg-gray-50/50'
                                  : hasRealSlots2 && hasAvailability2
                                    ? 'border-orange-300/60 bg-orange-50/25 hover:border-orange-400 hover:shadow-sm'
                                    : hasRealSlots2 && !hasAvailability2
                                      ? 'border-red-200/60 bg-red-50/20'
                                      : 'border-gray-200/80 bg-white hover:border-gray-300'
                          return (
                            ...
                            >
                              <div className="flex items-center justify-between px-1 sm:px-2.5 pt-1.5 sm:pt-2.5 pb-0.5 sm:pb-1 flex-shrink-0">
                                <span className={`text-[10px] sm:text-[13px] font-bold leading-none ${
                                  isSel2 ? 'text-green-600' : isDay1 ? 'text-orange-400' : isToday ? 'text-[#2563eb]' : 'text-gray-500'
                                }`}>{day}</span>
                                {isToday && !isDay1 && <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-[#2563eb] opacity-60 flex-shrink-0"></span>}
                                {isDay1 && <span className="text-[8px] font-black text-orange-400">D1</span>}
                              </div>

                              {/* Mobile: dot indicators only */}
                              <div className="flex sm:hidden flex-wrap gap-0.5 px-0.5 pb-1 pointer-events-none">
                                {daySlots2.slice(0, 4).map((slot) => {
                                  const isFullyBooked = slot.available_slots === 0;
                                  const isSlotSelected = promoPdcSlot2?.id === slot.id;
                                  return (
                                    <div
                                      key={slot.id}
                                      className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                                        isSlotSelected ? 'bg-green-500' :
                                        isFullyBooked ? 'bg-red-400' :
                                        (slot.session||'').toLowerCase().includes('morning') ? 'bg-orange-400' :
                                        (slot.session||'').toLowerCase().includes('afternoon') ? 'bg-yellow-400' :
                                        'bg-blue-400'
                      ...
                      <div className="flex sm:hidden flex-wrap justify-center gap-3 mt-4 pt-4 border-t border-gray-200">
                        ...
                        <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-blue-400"></div><span className="text-[10px] font-semibold text-gray-500">Whole Day</span></div>
                        <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-red-400"></div><span className="text-[10px] font-semibold text-gray-500">Full</span></div>
                        <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-green-500"></div><span className="text-[10px] font-semibold text-gray-500">Selected</span></div>
                      </div>
...
const isFull = slot.available_slots === 0
                  ...
                  return (
                    <button key={slot.id} onClick={() => handleSlotClick(slot)} disabled={isFull || isDay2} style={{ background: cardBg, borderColor: cardBorder }} className={`group relative flex flex-col p-4 text-left rounded-2xl border-2 transition-all overflow-hidden ${isFull ? 'opacity-60 cursor-not-allowed' : isDay2 ? 'shadow-xl shadow-green-500/30 scale-[1.02]' : 'hover:shadow-md hover:-translate-y-0.5'}`}>
                      <div
                        className="h-1.5 w-full flex-shrink-0"
                        style={{ background: isDay1 ? 'rgba(255,255,255,0.3)' : isDay2 ? '#22c55e' : sessionMeta.color }}
                      />
